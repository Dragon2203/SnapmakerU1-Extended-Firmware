...
7. When `openrfid-generic` is configured, non-Snapmaker filament metadata sent to Klipper
   uses `Generic` as the vendor and clears the variant. The resolved Spoolman object,
   spool ID, card UID, cache data, active spool tracking, and usage reporting remain
   unchanged.
